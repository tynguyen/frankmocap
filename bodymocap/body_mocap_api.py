...
from bodymocap.utils.imutils import (
    crop,
    crop_bboxInfo,
    process_image_bbox,
    process_image_keypoints,
    bbox_from_keypoints,
)
...
class BodyMocap(object):
    def __init__(
        ...
        if use_smplx:
            ...
            self.smpl = SMPLX(
                smpl_dir, batch_size=1, num_betas=10, use_pca=False, create_transl=False
            ).to(self.device)
            self.use_smplx = True
        else:
            smplModelPath = smpl_dir + "/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl"
            self.smpl = SMPL(smplModelPath, batch_size=1, create_transl=False).to(
                self.device
            )
    ...
    def regress(self, img_original, body_bbox_list):
        """
            args:
                ...
                body_bbox_list: list of body bounding boxes
            outputs:
                pred_vertices_img:
                pred_joints_vis_img:
                pred_rotmat
        ...
        for body_bbox in body_bbox_list:
            ...
            with torch.no_grad():
                ...
                pred_output["pred_vertices_smpl"] = (
                    smpl_output.vertices[0].cpu().numpy()
                )  # SMPL vertex in original smpl space
                pred_output[
                    "pred_vertices_img"
                ] = pred_vertices_img  # SMPL vertex in image space
                pred_output[
                    "pred_joints_img"
                ] = pred_joints_vis_img  # SMPL joints in image space

                pred_aa_tensor = gu.rotation_matrix_to_angle_axis(
                    pred_rotmat.detach().cpu()[0]
                )
                pred_output["pred_body_pose"] = (
                    pred_aa_tensor.cpu().numpy().reshape(1, 72)
                )  # (1, 72)

                pred_output["pred_rotmat"] = (
                    pred_rotmat.detach().cpu().numpy()
                )  # (1, 24, 3, 3)
                ...
                if self.use_smplx:
                    img_center = (
                        np.array((img_original.shape[1], img_original.shape[0])) * 0.5
                    )
                    # right hand
            ...
            if pred_body is None:
                ...
                for hand_type in hand_bbox:
                    ...
                    if pred_joints_vis_img is not None:
                        # get initial bbox
                        x0, x1 = (
                            np.min(pred_joints_vis_img[:, 0]),
                            np.max(pred_joints_vis_img[:, 0]),
                        )
                        y0, y1 = (
                            np.min(pred_joints_vis_img[:, 1]),
                            np.max(pred_joints_vis_img[:, 1]),
                        )
