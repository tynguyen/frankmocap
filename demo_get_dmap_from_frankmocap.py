...
def run_frank_mocap(args, bbox_detector, body_mocap, hand_mocap, visualizer):
    ...
    while True:
        ...
        res_img, res_dmap = visualizer.visualize(
            img_original_bgr,
...
def main():
    ...
    # It's important to get depth map. For now, this only works with pytorch3d
    if args.renderer_type == "pytorch3d":
        visualizer = Visualizer(args.renderer_type, is_get_dmap=True)
    else:
        print(
            f"[Err] This demo only works with pytorch3d renderer. Please change it accordingly (--renderer pytorch3d) !"
        )
        exit(1)
